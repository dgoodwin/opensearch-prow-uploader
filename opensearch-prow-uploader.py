...
if __name__ == "__main__":
    prow_job = sys.argv[1]
    files_to_upload = sys.argv[2:]
    print("using prowjob as index name: %s" % prow_job)
    print("uploading files %s" % files_to_upload)
    for fn in files_to_upload:
        ...
        while chunk := list(islice(iterator, chunk_size)):
            ...
            data_strs = []
            for item in chunk:
                index_line = {'index': {'_index': prow_job}}
                data_strs.append(json.dumps(index_line))
                transformed = transform(item)
                transformed['file'] = fn
                transformed['prowjob'] = prow_job
